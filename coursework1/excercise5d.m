%This script shows excercise5 d)
clear
...
for j = 1:200
    ...
    minError = 10000;
    for i = -6:1:3
        w_train = (x_train'*x_train + 10^(i)*100*eye(1)) \ x_train' * y_train; %perform ridge regression
    ...
    w_train_test = (x_train_final'*x_train_final + 10^(minParameter)*100*eye(1)) \ x_train_final' * y_train_final; %perform ridge regression
...
for j = 1:200
    ...
    x_train = x(1:8,:); %Split the data set into a training set of size 80
    y_train = y(1:8,:);
    x_validate = x(9:10,:); %Split the data set into a validation set of size 20
    y_validate = y(9:10,:);
    x_test = x(101:600,:); % Split the data set into a test set of size 500.
    y_test = y(101:600,:);
